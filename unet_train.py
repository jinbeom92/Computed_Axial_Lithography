...
# -----------------------------
from dataset.unet_dataset import ZSlicePairDataset
...
def build_model(cfg: Dict) -> UNET: # if you want to use U-Net, change "SVTR" to "UNET"
    mcfg = cfg["model"]
    bpcfg = cfg["bp"]
    return UNET(
        c1d=mcfg["c1d"],
...
def run_epoch(
    model: UNET, # if you want to use U-Net, change "SVTR" to "UNET"
    loader: DataLoader,
...
def main() -> None:
    ...
    for epoch in range(1, cfg["train"]["epochs"] + 1):
        ...
        if va["loss"] < best_val:
            import numpy as np
            np.save(rf"C:\Users\ICTCOC\Desktop\vam\unet\result\recon_opt_total.npy", ro.detach().cpu().numpy())
            best_val = va["loss"]
